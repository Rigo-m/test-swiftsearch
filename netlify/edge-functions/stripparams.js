      ...
      if (redirectedQueryString) {
        ...
        for (const [key, value] of url.searchParams.entries()) {
          if (params.get(key) !== value) {
            console.log(
              `Incoming and redirected query params are different: ${key}=${value} vs ${params.get(key)}`,
            );
            identical = false;
            break;
          }
        ...
        if (identical) {
          console.log(
            "Found identical query strings, stripping the redirect URL query string",
          );
          res.headers.set("location", location.split("?")[0]);
          return res;
        } else {
          console.log(
            "Incoming and redirected query params are different, not altering location",
          );
        }
      }
    }
